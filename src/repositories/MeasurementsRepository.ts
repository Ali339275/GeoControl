 ...
 async getMeasurementsForSingleSensor(
  ...
  if (!groups || groups.length === 0) {
    return { sensorMacAddress: sensorMac, stats: null, measurements: [] };
  }
  ...
  const measurements = filteredMeasurements.map(meas => {
    const isOutlier = stats !== null && (meas.value > stats.upperThreshold || meas.value < stats.lowerThreshold);
    return { createdAt: meas.createdAt, value: parseFloat(meas.value.toFixed(4)), isOutlier };
  });
  ...
  const measurementData = await this.getMeasurementsForSingleSensor(networkCode, gatewayMac, sensorMac, startDate, endDate);

  return {
    sensorMac,
    stats: measurementData.stats
      ? {
          startDate: formatWithOffset(measurementData.stats.startDate),
          endDate: formatWithOffset(measurementData.stats.endDate),
 ...
 async getOutliersForSingleSensor(
  ...
  const outlierMeasurements = measurementData.measurements.filter(m => m.isOutlier);

  return {
    sensorMacAddress: sensorMac,
    stats: measurementData.stats,
    measurements: outlierMeasurements,
  };
}
